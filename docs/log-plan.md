...
- [ ] `POST /api/v1/wristkit/sync` with a valid key inserts three rows
- [ ] `POST /api/wristkit-sync` (old path) does the same thing
- [ ] Wrong key returns 401, missing key returns 401
- [ ] `content-type: text/plain` returns 415
- [ ] 31 requests in five minutes returns 429 with a `retry-after` header
- [ ] Bad payload returns 400 with zod issues in the body
- [ ] Unknown path under `/api/v1` returns JSON 404, not an HTML error page
