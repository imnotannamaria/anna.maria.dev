...
A file tree of the site itself. Folders open and close, files link to the real routes, and the
...
`blog/[slug]` become a folder or disappear? Every one of those is an editorial call, and a
derived tree would need a config file to override them — which is the hand-written tree again,
plus a walker to maintain.
...
Postgres — so there is a real moment where the card is a skeleton, and that skeleton has been
on the site since the day the home page was split into slots.

`TreeCardSkeleton` is that frame, and it is the card: the same `.bento-card`, the same
`CardHead` with the same `N routes` meta — `siteTreeRouteCount()` is arithmetic over a
hand-written constant, so it is known without a query — the same 32px rows at the same indents,
the same footer. Only the words are grey.
...
Folders expand with a height transition and their children stagger in — but the stagger is
dropped past `STAGGER_LIMIT` children, because a delay multiplied across a long list stops
...
`useReducedMotion()` and returns the same state changes with no transition.
